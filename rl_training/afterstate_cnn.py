"""Afterstate value learning over raw board input — Stage 4 Branch A.

Differs from afterstate_dqn.py in two places:
- Input is the raw post-placement 20x10 binary board, not 4 hand-crafted features.
- Network is a small CNN -> MLP head -> scalar V (instead of pure 32x32 MLP).

Tests whether the afterstate paradigm or Nuno's hand-crafted features
[lines, holes, bumpiness, height] are doing the work. If a learned
representation over the raw board matches or beats the 4-feature baseline,
feature engineering is not the load-bearing part of Stage 3.5.

Same nuno reward, same target-net update cadence, same epsilon-greedy
enumeration over (rot, x) placements. Only the state representation changes.

Usage:
    python train.py --algo afterstate_cnn
    python train.py --algo afterstate_cnn --config rl_training/configs/afterstate_cnn.json
"""
...
DEFAULT_CONFIG = Path(__file__).parent / "configs" / "afterstate_cnn.json"
...
class BoardVNet(nn.Module):
    """CNN -> MLP mapping raw 20x10 binary board -> scalar V."""

    def __init__(self, conv_channels, fc_hidden, device):
        super().__init__()
        R, C = BOARD_SHAPE
        conv_layers, in_ch = [], 1
    ...
    def forward(self, x):
        ...
        if x.dim() == 2:        # [R, C]      -> single sample
            x = x.unsqueeze(0).unsqueeze(0)
        elif x.dim() == 3:      # [B, R, C]   -> add channel dim
            x = x.unsqueeze(1)
...
class Replay:
    """Stores (s, s', r, done). s and s' are [R, C] float32 boards."""
    ...
    def sample(self, batch_size):
        ...
        s = np.stack([b[0] for b in batch])         # [B, R, C]
        s_next = np.stack([b[1] for b in batch])    # [B, R, C]
...
def choose_action(game, net, eps):
    ...
    boards = np.stack([afterstates[a][0] for a in actions])  # [N, R, C]
    with torch.no_grad():
...
def run_episode(net, eps, replay, gamma, max_steps=10000):
    game = Tetris()
    game.next_block()
    current_state = np.zeros(BOARD_SHAPE, dtype=np.float32)  # empty board
    total_reward = 0.0
    total_lines = 0
    steps = 0
    while game.state != "gameover" and steps < max_steps:
        action, chosen_board = choose_action(game, net, eps)
        if action is None:
            break
        lines, done = apply_action(game, action)
        reward = nuno_reward(lines, done)
        if replay is not None:
            replay.push(current_state, chosen_board, reward, done)
        current_state = chosen_board
        total_reward += reward
        total_lines += lines
        steps += 1
        if done:
...
def run_trial(cfg, trial_episodes):
    ...
    target_net = BoardVNet(cfg["conv_channels"], cfg["fc_hidden"], DEVICE)
...
def main(max_epoch=None, config_path=None):
    ...
    print(f"Afterstate CNN config: {json.dumps(cfg, indent=2)}")

    run_name = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_dir = LOGS_DIR / "afterstate_cnn" / run_name
    log_dir.mkdir(parents=True, exist_ok=True)
    weights_dir = WEIGHTS_DIR / "afterstate_cnn" / run_name
